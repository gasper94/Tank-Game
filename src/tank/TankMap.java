...
/*Maps location of Tanks, power up, walls and unbreakble walls. */

/**
 *
 * @author Ulises
 */
public class TankMap extends AbstractGameModifier implements Observer {
	int start;
	Integer position;
	String filename;
    ...
    public TankMap(String filename){
	super();
		...
		try {
			...
			while(line!=null){
				for(int i = 0, n = line.length() ; i < n ; i++) { 
				    ...
				    if(c=='5'){
				    	world.addPowerUp(new PowerUp(new Point(i*32, h*32), 0, new TankDoubleShot()));
				    }
				}
				h++;
				line = level.readLine();
